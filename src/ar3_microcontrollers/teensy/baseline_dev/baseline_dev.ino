...
const int CAL_SPEED = 700; // motor steps per second
// DEFAULT { 1, 1, 1, 2, 1, 1 }
...
void calibrateJoints(int* calJoints)
{
  ...
  for (int i = 0; i < NUM_JOINTS; i++)
  {
    ...
    stepperJoints[i].setMinPulseWidth(100);
  }
  while (!calAllDone)
  {
    calAllDone = true;
    for (int i = 0; i < NUM_JOINTS; ++i)
    {
      ...
      if (!calJointsDone[i])
      {
        ...
        else
        {
          ...
          stepperJoints[i].setMinPulseWidth(0);
  ...
  while (STATE == STATE_TRAJ)
  {
    ...
    if (received == '\n')
    {
      ...
      {   
          ...
          int calJoint5[] = { 0, 0, 0, 0, 1, 0 }; // 000100
...
//        int calJoints[] = { 0, 1, 1, 0, 0, 0 }; // 111110
