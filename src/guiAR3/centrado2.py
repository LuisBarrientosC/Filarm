...
#    pub = rospy.Publisher('stop' , Float32, queue_size = 1)
#    global S, op
#    if op==1:
#        S=1
#    if op==0:
...
def callback1(data):
    ...
    if op1==0 and counter1==1: 

        if ypi < -10 and ypi>-100: #and M3 != 2 and M2<=1.4:
            M5 += -((ypi*0.01)/8) 
            planear3()
        ...
        if ypi > 10 and ypi <= 100  : #and M2 != -1 and M3< 1:
            M5 += -((ypi*0.01)/8)
            planear3()
